...
int main (int argc, char* argv[])
{
	 ...
	 * В реальной функции (не main) скорее всего количество входных полей будет фиксированным и такая проверка не понадобится.
	 * Проверка на то, что данные введены правильно (нет ничего лишнего), осуществляется в рамках функции конвертации символов ascii в цифры,
	 * проверка на корректность введённых данных будет осуществляться в функции наполнения буфера.
	 */
	...
	else
	{
		message.status = 0;	/* если не указан, на всякий случай принудительно инициализируем */
			/* Если указан - запоминаем, хотя в большинстве случаев он ничего не делает */
	}
