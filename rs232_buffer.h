...
/* максимальные значения для полей. Минимальные - нули */
#define MAX_TRKNO		0x7F	/* максимальное значение для номера колонки */
#define MAX_COM			0xF		/* максимальное */
#define MAX_PRC			999999	/* максимальное значение для цены */
#define MAX_VOL			999999	/* максимальное значение для объёма */
...
typedef unsigned char uint8;	/* в рамках большого проекта этот тип скорее всего будет уже определён */
...
struct Message
{
	unsigned int trkNo;		/* размер поля int принят для унификации. В реальном приложении имеет смысл оставить байт */
	unsigned int command;	/* размер поля int принят для унификации. В реальном приложении имеет смысл оставить байт */
	unsigned int price;
	unsigned int volume;
	unsigned int status;
};
